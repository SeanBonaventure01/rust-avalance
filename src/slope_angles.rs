...
pub mod slope_angles {
    mod slope_anlge_helpers {
        fn build_open_topo_get_request(west_lon: f32, east_lon: f32, north_lat: f32, south_lat: f32, api_key: &str) -> String
        {
    ...
    pub struct Avalanche_Terrain {
        avalanche_dataset: Option<gdal::Dataset>,
        pub geo_json_out: Option<geojson::GeoJseon::Feature>
    };

    impl Avalanche_Terrain {

        fn get_avalanche_dataset(original_dataset: gdal::Dataset) -> Result<gdal::Dataset, Box<dyn std::error::Error>> {
            let slope_angles: Vec<f32> = match compute_slope_angle_from_dataset(&original_dataset)?;
            let slope_dataset = match convert_slope_vector_to_dataset(&original_dataset, &slope_angles)?;
            let contours = match compute_contours_from_slope_angles(&slope_dataset)?;            
            ...
            Avalanche_Terrain {avalanche_dataset: Some(slope_dataset), geo_json_out: Some(geojson)}
        }

        pub fn from_lat_long(api_key: &str, (lat_north, long_east, lat_south, lon_west) : (f64, f64, f64, f64)) -> Self {
            let request = slope_angles::build_open_topo_get_request(lon_west, lon_east, lat_nor, lat_south, open_topo_api_key);
            let open_topo_req = reqwest::blocking::get(request);
            let Ok(response) = open_topo_req else {
                println!("Got an error return!");
                Avalanche_Terrain {avalanche_dataset: None, geo_json_out: None}
            };
            ...
            let result = match get_avalanche_dataset(gdal_dataset) {
                Ok(e) => e,
                Err(v) => Avalanche_Terrain {avalanche_dataset: None, geo_json_out: None}
            }
        }

        pub fn from_file(file_path: &str) -> Self {
            let Ok(gdal_dataset) = gdal::Dataset::open(Path::new(file_path)) else {
                println!("Failed to process dataset!");
                Avalanche_Terrain {avalanche_dataset: None, geo_json_out: None}
            };
        ...
        pub fn get_slope_angle_from_point((lat, lon) : (f64, f64)) -> Result<f32, Box<dyn std::error::Error>>
        {
